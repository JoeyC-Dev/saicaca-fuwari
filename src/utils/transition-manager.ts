...
class TransitionManager {
    ...
    if (listeners) {
      listeners.forEach(callback => {
        try {
          ...
          console.error(`Error in transition event handler for ${event}:`, error);
        }
      });
    }
  }
  ...
  async init(): Promise<void> {
    ...
    if (supportsViewTransitions) {
      ...
      console.log('[TransitionManager] View Transitions not supported, loading Swup fallback');
